...
export async function PUT(request: Request, { params }: { params: Promise<{ id: string }> }) {
  try {
    ...
    const current = db.prepare('SELECT sortie_id, done_at FROM inversions WHERE id = ? AND deleted_at IS NULL').get(id) as { sortie_id: number; done_at: string | null } | undefined;
    if (!current) {
    ...
    const recreatedId = (recreated.created as { id: number }).id;
    if (current.done_at) {
      db.prepare('UPDATE inversions SET done_at = ?, updated_at = datetime(\'now\') WHERE id = ?').run(current.done_at, recreatedId);
    }

    const updated = db.prepare(`${inversionSelect} WHERE i.id = ?`).get(recreatedId);
    logAudit({ actor: getActor(request), action: 'update', entityType: 'inversion', entityId: id, details: { previous_sortie_id: current.sortie_id, next_sortie_id: input.sortie_id, facture_reference: input.facture_reference, preserved_done_at: current.done_at } });
