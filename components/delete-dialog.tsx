  ...
  return (
    <div className="fixed inset-0 z-40 flex items-center justify-center bg-black/40 p-4" onClick={(e) => e.target === e.currentTarget && onClose()}>
      <div role="dialog" aria-modal="true" aria-labelledby="delete-sortie-title" className="w-full max-w-md rounded-xl bg-white p-6 shadow-xl">
        <h2 id="delete-sortie-title" className="mb-3 text-lg font-semibold text-gray-800">Supprimer cette sortie ?</h2>
        ...
        <p className="mt-2 text-sm text-red-500 font-medium">Cette action est irréversible.</p>
        <p className="mt-1 text-xs text-gray-400">Suppression logique uniquement : la donnée reste traçable côté base.</p>
